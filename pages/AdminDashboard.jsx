...
import { fetchUsers, incrementPage, setLoading, toggleHasMore } from '../Redux/Slices/userSlice';
...
const AdminDashboard = () => {
  ...
  const getUsers = async ()=>{
    ...
    try {
      const response = await axios.get(`https://67597b75099e3090dbe1d697.mockapi.io/api/users?page=${page}&limit=2`)
...
{hasMore && !loading && <div className='w-full flex justify-center'>
  <button onClick={()=>{getUsers()}} className='mt- 10 bg-blue-500 w-36 m-auto p-2 rounded-md my-10 text-white hover:bg-blue-800 transition-all hover:scale-110 '>Load more</button></div>}
